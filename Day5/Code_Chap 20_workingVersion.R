...
# AHM workshop at MSU, East Lansing
# 22-26 July 2024

# R code for the session on Integrated Models on Friday
# This is code from Chapter 20 in the book
# 'Applied Statistical Modeling for ecologists'
# by Kéry & Kellner, Elsevier, 2024


## Code from file Chap 20_v.09.docx
## Datetime: 2023-07-19 21:48:04
...
suppressMessages(library(ASMbook))
...
plot(elev2[C2>0], jitter(ztC2), pch = 16, xlab = 'Elevation (m)', ylab = 'Counts', frame = FALSE, ylim = range(c(C1, ztC2)), col = alpha('grey80', 1), main = 'Data sets 1 and 2')
...
plot(elev3, jitter(y, amount = 0.04), xlab = 'Elevation (m)', ylab = 'Detection/nondetection', axes = FALSE, pch = 16, col = alpha('grey60', 0.3), main = 'Data set 3')
axis(1)
axis(1, at = c(250, 750, 1250, 1750), tcl = -0.25, labels = NA)
...
out1 <- jags(bdata, inits, params, "model1.txt", n.iter=ni, n.burnin=nb, n.chains=nc, n.thin=nt, n.adapt=na, parallel=TRUE)
...
tmp <- cbind(truth = truth, tmp1[,1:2], out1$summary[2:3, 1:2])
colnames(tmp)[3:5] <- c("SE(MLE)", "Post.mean", "Post.sd")
print(tmp, 3)
...
# Call JAGS from R (ART <1 min), check convergence and summarize posteriors
out2 <- jags(bdata, inits, params, "model2.txt", n.iter = ni, n.burnin = nb, n.chains = nc, n.thin = nt, n.adapt = na, parallel = TRUE)
...
# Compare truth with likelihood and Bayesian solutions
tmp <- cbind(truth = truth, tmp2[,1:2], out2$summary[2:3, 1:2])
colnames(tmp)[3:5] <- c("SE(MLE)", "Post.mean", "Post.sd")
print(tmp, 3)
...
out3 <- jags(bdata, inits, params, "model3.txt", n.iter = ni, n.burnin = nb, n.chains = nc, n.thin = nt, n.adapt = na, parallel = TRUE)
...
# Compare truth with likelihood and Bayesian solutions
tmp <- cbind(truth = truth, tmp3[,1:2], out3$summary[2:3, 1:2])
colnames(tmp)[3:5] <- c("SE(MLE)", "Post.mean", "Post.sd")
print(tmp, 3)
...
NLLjoint <- function(param, y1, x1, y2, x2, y3, x3) {
  # Definition of elements in param vector 
  #(shared between data sets)
...
# Minimize NLLjoint
inits <- c(alpha = 0, beta = 0)
solJoint <- optim(inits, NLLjoint, y1 = C1, x1 = selev1, y2 = ztC2, x2 = selev2[C2>0], y3 = y, x3 = selev3, hessian = TRUE, method = 'BFGS')
...
# Bundle data
str(dataList <- list(C1 = C1, C2 = ztC2, y = y, nsites1 = nsites1, nsites2 = length(ztC2), nsites3 = nsites3, selev1 = selev1, selev2 = selev2[C2>0], selev3 = selev3))
...
out4 <- jags(dataList, inits, params, "model4.txt", n.iter = ni, n.burnin = nb, n.chains = nc, n.thin = nt, n.adapt = na, parallel = TRUE)
...
# Compare truth with likelihood and Bayesian solutions
tmp <- cbind(truth = truth, tmp4[,1:2], out4$summary[2:3, 1:2])
colnames(tmp)[3:5] <- c("SE(MLE)", "Post.mean", "Post.sd")
print(tmp, 3)
...
coda::traceplot(out20.4.3) # not shown
...
comp <- cbind(truth = truth, DIY = diy_est, JAGS = jags_est, NIMBLE = nimble_est, Stan = stan_est, TMB = tmb_est)
print(comp, 4)

          # truth     DIY    JAGS  NIMBLE    Stan     TMB
# log.lam  0.6931  0.6861  0.6864  0.6858  0.6847  0.6861
# beta    -2.0000 -1.9704 -1.9695 -1.9704 -1.9730 -1.9704
...
print(cbind(truth = truth, "MLE(Poisson)"= tmp1[,1], "MLE(ZTPois)" = tmp2[,1], "MLE(cloglogBern)" = tmp3[,1], "MLE(integrated)" = tmp4[,1]), 3)

# Compare ASEs from all 4 models (stacked sideways)
print(cbind("ASE(Poisson)" = tmp1[,2], "ASE(ZTPois)" = tmp2[,2], "ASE(cloglogBern)" = tmp3[,2], "ASE(integrated)" = tmp4[,2]), 3)
...
plot(1:4, all.mles, pch = 16, xlab = '', ylab = '', axes = FALSE, frame = FALSE, main = 'Abundance intercept', cex = 2, ylim = c(0.5, 0.9))
...
plot(1:4, all.mles, pch = 16, xlab = '', ylab = '', axes = FALSE, frame = FALSE, main = 'Abundance slope', cex = 2, ylim = c(-2.2, -1.8))
axis(1, at = 1:4, c('Model 1', 'Model 2', 'Model 3', 'Integrated model'), las = 2)
segments(1:4, all.lower.CL, 1:4, all.upper.CL, lwd = 1.5)
axis(2, las = 1)
